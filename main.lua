...
--local AirCrafts = workspace.Zhentor0Aircraft
local UIS = game:GetService("UserInputService")
local SettingsScript = Me.PlayerGui.SettingsGui.LocalScript
-- workspace.Zhentor0Aircraft
local nhook 
local GC = getgc()
...
for _, model in ipairs(playerAircraft:GetDescendants()) do
   if model:IsA("Model")  then
    ...
    local trailParts = {} -- Boş bir tablo oluşturuyoruz.
    
    for _, blockStdModel in ipairs(aircraft:GetDescendants()) do
        if blockStdModel.Name == "BlockStd" and blockStdModel:IsA("BasePart") then
            table.insert(trailParts, blockStdModel) -- Parça tabloya ekleniyor.
        end
    end
    ...
    local wing = workspace.Zhentor0.gay:GetChildren()
    if #wing <= 2 then
